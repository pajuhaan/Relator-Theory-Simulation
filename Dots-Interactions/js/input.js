...
let dragPrev = null; // { running:boolean, centerUpdate:boolean }
...
export function bindStageInput(state, dom, actions) {
    ...
    uiCanvas.addEventListener("pointerdown", (e) => {
        ...
        if (hit !== -1) {
            ...
            dragPrev = {
                ...
                centerUpdate: state.settings.centerUpdate
            };

            state.runtime.running = false;          // pause while dragging
            state.settings.centerUpdate = false;    // prevent center overwrite
    ...
    function endDrag() {
        ...
        if (dragPrev) {
            state.settings.centerUpdate = dragPrev.centerUpdate;
            state.runtime.running = dragPrev.running;
            dragPrev = null;
        }
